...
VERSION_PATH = os.path.join("athanor_mush", "VERSION.txt")
...
def package_data():
    ...
    """
    file_set = []
    for root, dirs, files in os.walk("athanor_mush"):
        for f in files:
            if ".git" in f.split(os.path.normpath(os.path.join(root, f))):
                # Prevent the repo from being added.
                continue
            file_name = os.path.relpath(os.path.join(root, f), "athanor_mush")
...
setup(
    ...
    author="Volund",
    maintainer="Volund",
    url="https://github.com/volundmush/athanor_mush",
    ...
    python_requires=">=3.7",
    project_urls={
        "Source": "https://github.com/volundmush/athanor_mush",
